...
import {
  Activity, Copy, Eye, EyeOff, Globe, KeyRound, Mic, MoreHorizontal, Pencil, Plus, RefreshCw, Search, Settings, ShieldCheck, Trash2
} from 'lucide-react'
...
export default function Admin({ page, auth }) {
  ...
  return (
    <div>
      ...
      {page === 'channels' && <Channels auth={auth} setMsg={setMsg} setErr={setErr} />}
...
function channelModelDisabled(m) { return m?.channelSupported === false || m?.id === 'mimo-v2.5-tts-voiceclone' }
function channelModelLabel(m) { return m?.channelSupported === false ? `${m.name || m.id}（不可用于上游渠道）` : (m.name || m.id) }
...
/* ====================== Channels ====================== */
function Channels({ auth, setMsg, setErr }) {
  const [items,setItems]=useState([]),[label,setLabel]=useState('AxonHub'),[baseUrl,setBaseUrl]=useState('http://127.0.0.1:40090/v1'),[apiKey,setApiKey]=useState(''),[weight,setWeight]=useState(1)
  const [addModels,setAddModels]=useState([]),[availableModels,setAvailableModels]=useState([]),[editing,setEditing]=useState(null),[editLabel,setEditLabel]=useState(''),[editBaseUrl,setEditBaseUrl]=useState(''),[editApiKey,setEditApiKey]=useState(''),[editWeight,setEditWeight]=useState(1),[editModels,setEditModels]=useState([])
  const [revealed,setRevealed]=useState({}),[showCreate,setShowCreate]=useState(false),[search,setSearch]=useState(''),[status,setStatus]=useState('all'),[tab,setTab]=useState('all'),[menu,setMenu]=useState(null),[upstreamModels,setUpstreamModels]=useState([]),[editUpstreamModels,setEditUpstreamModels]=useState([]),[modelAliases,setModelAliases]=useState({}),[editModelAliases,setEditModelAliases]=useState({})
  async function load(){try{setItems(await (await api('/api/admin/channels',{headers:auth})).json())}catch(e){setErr(e.message)}}
  async function loadModels(){try{setAvailableModels(await (await api('/api/admin/models',{headers:auth})).json()||[])}catch(e){}}
  async function fetchUpstreamModels(baseUrl, apiKey, edit=false, channelId='') {
    try {
      const data = await (await api('/api/admin/upstream-models',{method:'POST',headers:auth,body:JSON.stringify({baseUrl,apiKey,channelId})})).json()
      if (edit) setEditUpstreamModels(data||[])
      else setUpstreamModels(data||[])
    } catch (e) { setErr(e.message) }
  }
  useEffect(()=>{load();loadModels()},[])
  function toggleModel(list,id){const model=availableModels.find(m=>m.id===id);if(channelModelDisabled(model))return list.filter(m=>m!==id);return list.includes(id)?list.filter(m=>m!==id):[...list,id]}
  async function add(){try{await api('/api/admin/channels',{method:'POST',headers:auth,body:JSON.stringify({label,baseUrl,apiKey,weight,models:addModels.length?addModels:['mimo-v2.5-tts','mimo-v2.5-tts-voicedesign'],modelAliases})});setApiKey('');setAddModels([]);setShowCreate(false);setMsg('已添加上游渠道');await load()}catch(e){setErr(e.message)}}
  async function del(id){if(!confirm('确认删除这个上游渠道？'))return;try{await api(`/api/admin/channels/${id}`,{method:'DELETE',headers:auth});await load()}catch(e){setErr(e.message)}}
  async function toggle(id,en){try{await api(`/api/admin/channels/${id}`,{method:'PATCH',headers:auth,body:JSON.stringify({enabled:!en})});await load()}catch(e){setErr(e.message)}}
  function startEdit(x){setEditing(editing===x.id?null:x.id);setEditLabel(x.label||'');setEditBaseUrl(x.baseUrl||'');setEditApiKey('');setEditWeight(x.weight||1);setEditModels(x.models||[]);setEditModelAliases(x.modelAliases||{});setMenu(null);fetchUpstreamModels(x.baseUrl||'', '', true, x.id)}
  async function saveEdit(id){try{const body={label:editLabel,baseUrl:editBaseUrl,weight:editWeight,models:editModels,modelAliases:editModelAliases};if(editApiKey.trim())body.apiKey=editApiKey;await api(`/api/admin/channels/${id}`,{method:'PATCH',headers:auth,body:JSON.stringify(body)});setEditing(null);setMsg('渠道已更新');await load()}catch(e){setErr(e.message)}}
  async function reveal(id){if(revealed[id]){setRevealed(r=>({...r,[id]:''}));return}if(!confirm('完整 API Key 属于敏感信息，确认显示？'))return;try{const d=await (await api(`/api/admin/channels/${id}/secret`,{headers:auth})).json();setRevealed(r=>({...r,[id]:d.secret||''}))}catch(e){setErr(e.message)}}
  const rows=items.filter(x=>{const q=search.trim().toLowerCase();return(!q||[x.id,x.label,x.baseUrl,x.masked].filter(Boolean).some(v=>String(v).toLowerCase().includes(q)))&&(status==='all'||(status==='enabled'?x.enabled:!x.enabled))&&(tab==='all'||(tab==='active'?x.enabled:!x.enabled))})
  return <PageShell icon={<Globe size={22}/>} title="上游渠道" desc="管理 OpenAI 兼容上游，统一配置模型、权重和密钥。上游渠道不支持音色克隆，需要官方 MiMo API Key 才能使用克隆音色。" count={items.length} activeCount={items.filter(x=>x.enabled).length} action={<button className="orange-btn" onClick={()=>setShowCreate(!showCreate)}><Plus size={15}/> 创建渠道</button>}>
    <SegmentTabs value={tab} onChange={setTab} tabs={[{value:'all',label:'全部'},{value:'active',label:'已启用'},{value:'disabled',label:'已禁用'}]}/>
    {showCreate&&<div className="drawer-card"><div className="drawer-title">新增上游渠道</div><div className="drawer-grid"><label>名称<input value={label} onChange={e=>setLabel(e.target.value)}/></label><label className="wide">Base URL<input value={baseUrl} onChange={e=>setBaseUrl(e.target.value)}/></label><label>API Key<input value={apiKey} onChange={e=>setApiKey(e.target.value)} type="password" placeholder="可选"/></label><label>权重<input type="number" min="1" max="100" value={weight} onChange={e=>setWeight(Number(e.target.value))}/></label><button className="primary" onClick={add}>保存</button><button className="smallbtn" onClick={()=>fetchUpstreamModels(baseUrl, apiKey)}><RefreshCw size={14}/> 读取上游模型</button></div><div className="model-checks compact">{availableModels.map(m=><label key={m.id} className={`model-check ${channelModelDisabled(m)?'disabled':''}`} title={m.channelNote||''}><input type="checkbox" disabled={channelModelDisabled(m)} checked={!channelModelDisabled(m)&&addModels.includes(m.id)} onChange={()=>setAddModels(toggleModel(addModels,m.id))}/><span>{channelModelLabel(m)}</span></label>)}</div>{upstreamModels.length>0 && <div className="alias-grid">{availableModels.filter(m=>m.id!=='*').map(m=><label key={m.id}>{standardModelLabel(m.id)}<select value={modelAliases[m.id]||''} onChange={e=>setModelAliases(v=>({...v,[m.id]:e.target.value}))}><option value=''>未映射</option>{upstreamModels.map(u=><option key={u.id} value={u.id}>{u.name||u.id}</option>)}</select></label>)}</div>}</div>}
    <ListToolbar search={search} setSearch={setSearch} status={status} setStatus={setStatus} placeholder="按名称、URL、ID 搜索..."/>
    <div className="table-card table-card-open"><table className="admin-table"><thead><tr><th>ID</th><th>渠道</th><th>Base URL</th><th>API Key</th><th>模型</th><th>权重</th><th>状态</th><th>操作</th></tr></thead><tbody>{rows.map(x=><React.Fragment key={x.id}><tr><td className="mono muted">#{x.id}</td><td><b>{x.label}</b></td><td className="mono url-cell">{x.baseUrl}</td><td><div className="secret-cell"><span>{revealed[x.id]||maskSecret(x.masked)}</span><button onClick={()=>reveal(x.id)}>{revealed[x.id]?<EyeOff size={14}/>:<Eye size={14}/>}</button><button onClick={()=>copyText(revealed[x.id]||x.masked,setMsg)}><Copy size={14}/></button></div></td><td><div className="model-tags">{Array.isArray(x.models)&&x.models.length?x.models.slice(0,2).map(m=><span key={m}>{standardModelLabel(m)}</span>):<span>默认</span>}{Array.isArray(x.models)&&x.models.length>2&&<span>+{x.models.length-2}</span>}</div><div className="subtle">{x.modelAliases && Object.keys(x.modelAliases).length ? Object.entries(x.modelAliases).map(([k,v])=>`${standardModelLabel(k)} → ${v}`).join('；') : '未配置上游模型映射'}</div></td><td>{x.weight}</td><td><span className={x.enabled?'status-text ok':'status-text off'}>{x.enabled?'已启用':'已禁用'}</span></td><td className="op-cell"><button className="dots" onClick={()=>setMenu(menu===x.id?null:x.id)}><MoreHorizontal size={18}/></button><RowMenu open={menu===x.id}><button onClick={()=>startEdit(x)}><Pencil size={14}/> 编辑</button><button onClick={()=>toggle(x.id,x.enabled)}><Settings size={14}/> {x.enabled?'禁用':'启用'}</button><button onClick={()=>reveal(x.id)}><Eye size={14}/> {revealed[x.id]?'隐藏密钥':'显示密钥'}</button><button className="danger" onClick={()=>del(x.id)}><Trash2 size={14}/> 删除</button></RowMenu></td></tr>{editing===x.id&&<tr className="edit-row"><td colSpan="8"><div className="inline-edit"><label>名称<input value={editLabel} onChange={e=>setEditLabel(e.target.value)}/></label><label className="wide">Base URL<input value={editBaseUrl} onChange={e=>setEditBaseUrl(e.target.value)}/></label><label>API Key<input value={editApiKey} onChange={e=>setEditApiKey(e.target.value)} placeholder="留空不修改" type="password"/></label><label>权重<input type="number" min="1" max="100" value={editWeight} onChange={e=>setEditWeight(Number(e.target.value))}/></label><button className="primary" onClick={()=>saveEdit(x.id)}>保存</button><button className="btn" onClick={()=>setEditing(null)}>取消</button><button className="smallbtn" onClick={()=>fetchUpstreamModels(editBaseUrl, editApiKey, true, x.id)}><RefreshCw size={14}/> 读取上游模型</button></div><div className="model-checks compact">{availableModels.map(m=><label key={m.id} className={`model-check ${channelModelDisabled(m)?'disabled':''}`} title={m.channelNote||''}><input type="checkbox" disabled={channelModelDisabled(m)} checked={!channelModelDisabled(m)&&editModels.includes(m.id)} onChange={()=>setEditModels(toggleModel(editModels,m.id))}/><span>{channelModelLabel(m)}</span></label>)}</div>{editUpstreamModels.length>0 && <div className="alias-grid">{availableModels.filter(m=>m.id!=='*').map(m=><label key={m.id}>{standardModelLabel(m.id)}<select value={editModelAliases[m.id]||''} onChange={e=>setEditModelAliases(v=>({...v,[m.id]:e.target.value}))}><option value=''>未映射</option>{editUpstreamModels.map(u=><option key={u.id} value={u.id}>{u.name||u.id}</option>)}</select></label>)}</div>}</td></tr>}</React.Fragment>)}{rows.length===0&&<tr><td colSpan="8" className="empty-cell">暂无上游渠道</td></tr>}</tbody></table></div>
  </PageShell>
}
